...
describe HypatiaCollContentMetadataDS do
  ...
  it "should hook to the proper EAD FileAsset object and have correct metadata for that object" do
    ...
    @content_md_ds.term_values(:ead_ds_id).should == ["DS1"]
    @content_md_ds.term_values(:ead_filename).should == ["coll_ead.xml"]  # must match LABEL attribute of datastream in EAD FileAsset object
    @content_md_ds.term_values(:ead_size).should == ["47570"]
    @content_md_ds.term_values(:ead_mimetype).should == ["text/xml"]
  ...
  it "should hook to the proper image FileAsset object and have correct metadata for that object" do
    ...
    @content_md_ds.term_values(:image_ds_id).should == ["DS1"]
    @content_md_ds.term_values(:image_filename).should == ["fixture_coll_image.jpg"]  # must match LABEL attribute of datastream in coll image FileAsset object
    @content_md_ds.term_values(:image_size).should == ["302080"]
    @content_md_ds.term_values(:image_mimetype).should == ["image/jpeg"]
