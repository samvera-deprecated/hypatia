...
class DorIdentityMetadataDS < ActiveFedora::NokogiriDatastream 
  ...
  set_terminology do |t|
    ...
    t.hypatiaId(:path=>"sourceId", :attributes=>{:source=>"hypatia"}, :index_as=>[:searchable,  :displayable, :facetable, :sortable], :required=>:true, :type=>:string, :namespace_prefix => nil )  
    ...
    t.uuid(:path=>"otherId", :attributes=>{:name=>"uuid"}, :index_as=>[:searchable,  :displayable, :facetable, :sortable], :required=>:true, :type=>:string, :namespace_prefix => nil )  
  end
